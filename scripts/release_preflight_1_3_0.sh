...
NOTE="1.3.0 readiness preflight"
SKIP_VERIFY=false
SKIP_DOC_GATES=false

usage() {
  cat <<EOF_USAGE
$SCRIPT_NAME - 1.3.0 Readiness-Preflight ausfuehren
...
Options:
  ...
  --skip-doc-gates     ueberspringt Gate-Status-/Doku-Guardrails aus den 1.3.0-Dokumenten
...
Beispiele:
  ...
  $SCRIPT_NAME --note "Preflight vor 1.3.0 Freigabe"
EOF_USAGE
}
...
if [[ "$APP_VERSION" != "1.3.0-dev" ]]; then
  die "Preflight 1.3.0 erwartet APP_VERSION=1.3.0-dev (gefunden: $APP_VERSION)."
fi
ok "Versionierungs-Guardrail passt (1.3.0-dev aktiv)."

if ! $SKIP_DOC_GATES; then
  info "Pruefe Gate-Status-/Doku-Guardrails fuer 1.3.0."

  check_doc_gate \
    "$ROOT_DIR/docs/BACKLOG.md" \
    'BL-0017 .*Status: `done`' \
    "Backlog-Index fuehrt BL-0017 als done"

  check_doc_gate \
    "$ROOT_DIR/docs/BACKLOG.md" \
    'BL-0018 .*Status: `done`' \
    "Backlog-Index fuehrt BL-0018 als done"

  check_doc_gate \
    "$ROOT_DIR/docs/backlog/BL-0017-fuel-price-api-evaluation/item.md" \
    '^status:\s*done$' \
    "BL-0017 ist auf done gesetzt"

  check_doc_gate \
    "$ROOT_DIR/docs/backlog/BL-0018-fuel-price-history-polling/item.md" \
    '^status:\s*done$' \
    "BL-0018 ist auf done gesetzt"

  check_doc_gate \
    "$ROOT_DIR/docs/backlog/BL-0018-fuel-price-history-polling/tasks/TSK-0021-implement-polling-baseline-persistence-and-regression-evidence.md" \
    '^status:\s*done$' \
    "TSK-0021 ist auf done gesetzt"

  check_doc_gate \
    "$ROOT_DIR/docs/ROADMAP_1_3_0.md" \
    'Gate 3: Umsetzung \+ Contract-Haertung' \
    "Roadmap enthaelt Gate-3-Block"

  check_doc_gate \
    "$ROOT_DIR/docs/ROADMAP_1_3_0.md" \
    'Status:[[:space:]]*$' \
    "Roadmap enthaelt Status-Abschnitte"

  check_doc_gate \
    "$ROOT_DIR/docs/ROADMAP_1_3_0.md" \
    'abgeschlossen am 2026-03-24\.' \
    "Roadmap dokumentiert Gate-3-Abschluss"
  ...
  check_doc_gate \
    ...
    '\*\*Status:\*\* aktiv' \
    "Release-Preflight 1.3.0 ist als aktiv dokumentiert"
else
...
ok "1.3.0 Readiness-Preflight abgeschlossen."
