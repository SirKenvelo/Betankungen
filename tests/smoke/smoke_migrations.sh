...
Usage:
  tests/smoke/smoke_migrations.sh [--v4-to-v5] [--all] [-l|--list] [-h|--help]

Optionen:
  --v4-to-v5    Fuehrt nur den Migrations-Check v4 -> v5 aus
...
print_plan() {
  ...
  if $RUN_V4_TO_V5; then
    printf '[LIST] v4 -> v5: ALTER-Migration fuer cars(vin/reg_doc_*) + schema_version=5 + idempotenter Re-Run\n'
  fi
...
run_v4_to_v5_check() {
  local db="$TMP_DIR/v4_to_v5.db"
  local rc schema_version cars_count preserved_row
  ...
  if [[ $rc -ne 0 ]]; then
    fail 'v4 -> v5: App-Start zur Migration fehlgeschlagen.'
  fi

  schema_version="$(sqlite3 "$db" "SELECT value FROM meta WHERE key='schema_version' LIMIT 1;")"
  if [[ "$schema_version" != "5" ]]; then
    fail "v4 -> v5: schema_version erwartet=5, ist=${schema_version:-<leer>}"
  fi
  ...
  if [[ "$cars_count" != "1" ]]; then
    fail "v4 -> v5: erwartet genau 1 Car-Datensatz, ist=${cars_count}"
  fi
  ...
  if [[ "$preserved_row" != "FixtureCar|F-123|legacy-note|123|2020-01-01" ]]; then
    fail "v4 -> v5: bestehende Car-Daten wurden unerwartet veraendert (${preserved_row})"
  fi
  ...
  if [[ "$(sqlite3 "$db" "SELECT COALESCE(vin, '<null>') || '|' || COALESCE(reg_doc_path, '<null>') || '|' || COALESCE(reg_doc_sha256, '<null>') FROM cars WHERE id=1;")" != "<null>|<null>|<null>" ]]; then
    fail 'v4 -> v5: neue v5-Spalten sollten fuer Legacy-Row initial NULL sein.'
  fi

  set +e
  "$APP_BIN" --db "$db" --list cars >"$TMP_DIR/rerun.out" 2>"$TMP_DIR/rerun.err"
  rc=$?
  set -e
  if [[ $rc -ne 0 ]]; then
    fail 'v4 -> v5: idempotenter Re-Run fehlgeschlagen.'
  fi

  if [[ "$(sqlite3 "$db" "SELECT value FROM meta WHERE key='schema_version' LIMIT 1;")" != "5" ]]; then
    fail 'v4 -> v5: schema_version ist nach Re-Run nicht stabil auf 5.'
  fi

  printf '[OK] v4 -> v5: ALTER-Migration + schema_version + Idempotenz geprueft\n'
}
