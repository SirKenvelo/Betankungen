...
# t_p001__02__car_id_negative.sh
# UPDATED: 2026-03-01
# Policy P-001: explizites --car-id -1 muss als Hard Error abbrechen (kein Write).
...
TMP_DIR="$(mktemp -d /tmp/t_p001_negative_XXXXXX)"
...
set +e
"$APP_BIN" --db "$DB_POLICY" --add fuelups --car-id -1 >"$OUT_FILE" 2>"$ERR_FILE" </dev/null
RC=$?
set -e

if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer --car-id -1, erhalten: 0'
fi
...
printf '[OK] P-001/02: --car-id -1 fuehrt zu Hard Error ohne Insert.\n'
