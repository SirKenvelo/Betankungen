...
# t_p081__01__station_zip_non_numeric_rejected.sh
# UPDATED: 2026-03-31
# Policy P-081: zip muss numerisch sein.
...
TMP_DIR="$(mktemp -d /tmp/t_p081_zip_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-081, erhalten: 0'
fi

if ! grep -q 'P-081' "$ERR_FILE"; then
  fail 'Erwartete P-081-Fehlermeldung nicht gefunden.'
fi
...
printf '[OK] P-081/01: nicht-numerische zip wird sauber geblockt.\n'
