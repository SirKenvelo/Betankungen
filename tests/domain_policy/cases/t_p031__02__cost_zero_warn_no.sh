...
# t_p031__02__cost_zero_warn_no.sh
# UPDATED: 2026-02-22
# Policy P-031: cost_cents == 0 mit Confirm=NO bricht ab und speichert nicht.
...
TMP_DIR="$(mktemp -d /tmp/t_p031_no_XXXXXX)"
...
if ! grep -q 'P-031' "$ERR_FILE"; then
  fail 'Erwartete P-031-Fehlermeldung nicht gefunden.'
fi

if ! grep -q 'Abbruch durch Benutzer (Gesamtpreis=0).' "$ERR_FILE"; then
  fail 'Erwartete Abbruchmeldung fuer P-031 nicht gefunden.'
fi
...
printf '[OK] P-031/02: Confirm=NO bricht bei Gesamtpreis=0 ab und speichert nicht.\n'
