...
# t_p022__02__consumption_warn_no.sh
# UPDATED: 2026-02-22
# Policy P-022: Bei sehr grosser Tankmenge und Confirm=NO muss der Add-Flow abbrechen (kein Insert).
...
TMP_DIR="$(mktemp -d /tmp/t_p022_no_XXXXXX)"
...
if ! grep -q 'Abbruch durch Benutzer (Tankmenge).' "$ERR_FILE"; then
  fail 'Erwartete Abbruchmeldung fuer Tankmengen-Warnung nicht gefunden.'
fi

COUNT_ROW="$(sqlite3 "$DB_POLICY" "SELECT COUNT(*) FROM fuelups WHERE odometer_km = 10100;")"
if [[ "$COUNT_ROW" != "0" ]]; then
  fail "Erwartet keinen Datensatz fuer odometer_km=10100, erhalten: $COUNT_ROW"
fi

printf '[OK] P-022/02: Confirm=NO bricht bei Tankmengen-Warnung ab und speichert nicht.\n'
