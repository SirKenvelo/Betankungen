...
# t_p085__01__station_geodata_pair_required.sh
# UPDATED: 2026-03-31
# Policy P-085: latitude/longitude muessen als Paar gesetzt werden.
...
TMP_DIR="$(mktemp -d /tmp/t_p085_geo_pair_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-085, erhalten: 0'
fi

if ! grep -q 'P-085' "$ERR_FILE"; then
  fail 'Erwartete P-085-Fehlermeldung nicht gefunden.'
fi
...
printf '[OK] P-085/01: unvollstaendiges Koordinatenpaar wird sauber geblockt.\n'
