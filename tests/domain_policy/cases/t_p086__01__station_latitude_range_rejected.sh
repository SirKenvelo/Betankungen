...
# t_p086__01__station_latitude_range_rejected.sh
# UPDATED: 2026-03-31
# Policy P-086: latitude muss im gueltigen Dezimalgrad-Bereich liegen.
...
TMP_DIR="$(mktemp -d /tmp/t_p086_lat_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-086, erhalten: 0'
fi

if ! grep -q 'P-086' "$ERR_FILE"; then
  fail 'Erwartete P-086-Fehlermeldung nicht gefunden.'
fi
...
printf '[OK] P-086/01: latitude ausserhalb des Bereichs wird sauber geblockt.\n'
