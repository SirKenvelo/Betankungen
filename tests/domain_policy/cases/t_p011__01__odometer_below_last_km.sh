...
# t_p011__01__odometer_below_last_km.sh
# UPDATED: 2026-02-22
# Policy P-011: odometer_km <= last_odometer_km (pro car) fuehrt bei kleinerem Wert zu Hard Error ohne Write.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
DB_POLICY="$ROOT_DIR/tests/domain_policy/fixtures/Betankungen_Policy.db"
FIXTURE_SQL="$ROOT_DIR/tests/domain_policy/fixtures/p011_base.sql"
DB_BUILDER="$ROOT_DIR/tests/domain_policy/helpers/build_test_dbs.sh"
APP_BIN="$ROOT_DIR/bin/Betankungen"

TMP_DIR="$(mktemp -d /tmp/t_p011_last_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-011, erhalten: 0'
fi

if ! grep -q 'P-011' "$ERR_FILE"; then
  fail 'Erwartete P-011-Fehlermeldung nicht gefunden.'
fi

if grep -q 'P-013' "$ERR_FILE"; then
  fail 'Fehler wurde faelschlich als P-013 klassifiziert.'
fi
...
printf '[OK] P-011/01: Odometer unter Last-KM fuehrt zu Hard Error ohne Insert.\n'
