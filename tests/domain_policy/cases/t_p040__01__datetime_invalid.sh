...
# t_p040__01__datetime_invalid.sh
# UPDATED: 2026-02-22
# Policy P-040: Datum/Zeit fehlt/ungueltig fuehrt zu Hard Error ohne Write.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
DB_POLICY="$ROOT_DIR/tests/domain_policy/fixtures/Betankungen_Policy.db"
FIXTURE_SQL="$ROOT_DIR/tests/domain_policy/fixtures/p040_base.sql"
DB_BUILDER="$ROOT_DIR/tests/domain_policy/helpers/build_test_dbs.sh"
APP_BIN="$ROOT_DIR/bin/Betankungen"

TMP_DIR="$(mktemp -d /tmp/t_p040_invalid_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-040, erhalten: 0'
fi

if ! grep -q 'P-040' "$ERR_FILE"; then
  fail 'Erwartete P-040-Fehlermeldung nicht gefunden.'
fi
...
printf '[OK] P-040/01: ungueltiges Datum/Zeit-Format fuehrt zu Hard Error ohne Insert.\n'
