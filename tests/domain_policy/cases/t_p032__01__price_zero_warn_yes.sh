...
# t_p032__01__price_zero_warn_yes.sh
# UPDATED: 2026-02-22
# Policy P-032: price_cents_per_liter <= 0 mit Confirm=YES speichert normal.
...
TMP_DIR="$(mktemp -d /tmp/t_p032_yes_XXXXXX)"
...
if ! grep -q 'P-032: Warnung: Preis/Liter <= 0' "$OUT_FILE"; then
  fail 'Erwartete P-032-Warnung nicht gefunden.'
fi
...
PPL_MILLI="$(sqlite3 "$DB_POLICY" "SELECT price_per_liter_milli_eur FROM fuelups WHERE odometer_km = 10100 LIMIT 1;")"
if [[ "$PPL_MILLI" != "0" ]]; then
  fail "Erwartet price_per_liter_milli_eur=0 fuer odometer_km=10100, erhalten: ${PPL_MILLI:-<leer>}"
fi

printf '[OK] P-032/01: Confirm=YES speichert bei Preis/Liter=0 normal.\n'
