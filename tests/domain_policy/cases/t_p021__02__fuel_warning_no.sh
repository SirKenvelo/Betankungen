...
# t_p021__02__fuel_warning_no.sh
# UPDATED: 2026-02-22
# Policy P-021: liters > 150 mit Confirm=NO bricht ab und speichert nicht.
...
TMP_DIR="$(mktemp -d /tmp/t_p021_no_XXXXXX)"
...
if ! grep -q 'P-021' "$ERR_FILE"; then
  fail 'Erwartete P-021-Fehlermeldung nicht gefunden.'
fi

if ! grep -q 'Abbruch durch Benutzer (Tankmenge).' "$ERR_FILE"; then
  fail 'Erwartete Abbruchmeldung fuer P-021 nicht gefunden.'
fi
...
printf '[OK] P-021/02: Confirm=NO bricht bei Fuel-Warnung ab und speichert nicht.\n'
