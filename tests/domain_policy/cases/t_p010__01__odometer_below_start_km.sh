...
# t_p010__01__odometer_below_start_km.sh
# UPDATED: 2026-02-22
# Policy P-010: odometer_km < cars.odometer_start_km fuehrt zu Hard Error ohne Write.
...
TMP_DIR="$(mktemp -d /tmp/t_p010_start_XXXXXX)"
...
if [[ $RC -eq 0 ]]; then
  fail 'Erwartet Exitcode != 0 fuer P-010, erhalten: 0'
fi

if ! grep -q 'P-010' "$ERR_FILE"; then
  fail 'Erwartete P-010-Fehlermeldung nicht gefunden.'
fi

if grep -q 'P-011\|P-013' "$ERR_FILE"; then
  fail 'Fehler wurde faelschlich als P-011/P-013 klassifiziert.'
fi
...
printf '[OK] P-010/01: Start-KM-Verletzung fuehrt zu Hard Error ohne Insert.\n'
